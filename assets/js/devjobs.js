...
function handleFilter() {
  ...
  const fullTimeFilter = document.querySelector('.fullTimeFilter');

  const searchTitleFilter = titleFilter.value.trim().toLowerCase();
  const searchLocationFilter = locationFilter.value.trim().toLowerCase();
  const searchfullTimeFilter = fullTimeFilter.checked;

  console.log(searchTitleFilter);
  console.log(searchLocationFilter);
  console.log(searchfullTimeFilter);

  const filteredData = jobs
    .filter(x => {
      const matchesInput = !searchTitleFilter ||
        x.position.toLowerCase().includes(searchTitleFilter) ||
        x.company.toLowerCase().includes(searchTitleFilter) ||
        x.postedAt.toLowerCase().includes(searchTitleFilter)

      const matchesLocation = !searchLocationFilter ||
        x.location.toLowerCase().includes(searchLocationFilter)

      const matchesFullTime = !searchfullTimeFilter ||
        x.contract === "Full Time"
      return matchesInput && matchesLocation && matchesFullTime;
    }
    )
  render(filteredData);
}
...
function render(filteredWord) {
  jobList.innerHTML = filteredWord.map
    (x => `<li class="listItem">
              <div class="listLogo"><img src="${x.logo}"></div> 
